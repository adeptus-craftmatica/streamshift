...
class OBSStudioPlugin:
    ...
    def _open_deck_page(self, page: str) -> None:
    ...
    def _connection_config(self) -> ObsConnectionConfig:
        ...
        timeout = max(1, self._setting_int("timeout_seconds", 3))

        # Always try auto-discovery first — OBS config file has the definitive
        # port and password, and the host is always localhost (OBS WebSocket
        # runs on the same machine). This means the connection never breaks
        # when the IP changes.
        discovered = discover_obs_websocket()
        if discovered is not None:
            host, port, password = discovered
        ...
        # Fallback: use whatever the user has entered in Settings.
        return ObsConnectionConfig(
