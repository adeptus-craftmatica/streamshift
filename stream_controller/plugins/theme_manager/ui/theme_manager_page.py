...
class ThemeManagerPage(QWidget):
    ...
    def _build_left_panel(self) -> QWidget:
        ...
        # Built-in label
        bi_lbl = QLabel("  BUILT-IN"); bi_lbl.setObjectName("SidebarSectionTitle")
        bi_lbl.setContentsMargins(14, 10, 14, 4)
        vl.addWidget(bi_lbl)

        self._builtin_list = QListWidget()
        self._builtin_list.setObjectName("SidebarList")
        self._builtin_list.setMaximumHeight(230)
        self._builtin_list.currentItemChanged.connect(lambda c, p: self._on_theme_selected(c))
        vl.addWidget(self._builtin_list)

        custom_lbl = QLabel("  CUSTOM"); custom_lbl.setObjectName("SidebarSectionTitle")
        custom_lbl.setContentsMargins(14, 8, 14, 4)
        vl.addWidget(custom_lbl)

        self._custom_list = QListWidget()
        self._custom_list.setObjectName("SidebarList")
        self._custom_list.currentItemChanged.connect(lambda c, p: self._on_theme_selected(c))
        vl.addWidget(self._custom_list, 1)
    ...
    def _build_center_panel(self) -> QWidget:
        ...
        # Top bar: theme name + apply button
        top = QWidget(); top.setObjectName("SidebarHeader")
        tl = QHBoxLayout(top); tl.setContentsMargins(16, 12, 16, 12); tl.setSpacing(10)
        self._theme_name_lbl = QLabel("Select a theme"); self._theme_name_lbl.setObjectName("CardTitle")
        tl.addWidget(self._theme_name_lbl, 1)
        rename_btn = QPushButton("Rename"); rename_btn.setObjectName("SecondaryButton")
        rename_btn.setFixedWidth(70); rename_btn.clicked.connect(self._rename_theme)
        self._apply_btn = QPushButton("⚡ Apply Theme"); self._apply_btn.setObjectName("PrimaryButton")
        self._apply_btn.clicked.connect(self._apply_and_save)
        self._apply_btn.setEnabled(False)
        tl.addWidget(rename_btn); tl.addWidget(self._apply_btn)
        vl.addWidget(top)
    ...
    def _refresh_theme_list(self) -> None:
        ...
        # Select active theme if nothing selected
