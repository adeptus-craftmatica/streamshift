...
class SceneManagerPlugin:
    ...
    def register(self, app_context: AppContext) -> None:
        ...
        if self._repo.get("auto_connect"):
            self._retry_count = 0
            host, port, password = self._resolve_obs_creds()
    ...
    def do_connect(self) -> None:
        """Trigger a connection attempt — callable from Connect All."""
    ...
    def is_connected(self) -> bool:
        if self._client is None:
            return False
        return self._client.state.status == ConnectionStatus.CONNECTED
    ...
    def _start_retry_timer(self) -> None:
        ...
        self._retry_timer.setInterval(4000)
    ...
    def _resolve_obs_creds(self) -> tuple[str, int, str]:
        """Return (host, port, password) for the OBS WebSocket connection.

        Priority:
          1. OBS's own config file — always current, no user setup needed.
          2. Scene Manager's stored settings — fallback if OBS isn't installed
             locally or the config file can't be read.
        """
        obs_cfg = discover_obs_websocket()
        if obs_cfg is not None:
            host, port, password = obs_cfg
            logger.debug(
                "Scene Manager: auto-discovered OBS WebSocket (port=%d, auth=%s)",
                port, bool(password),
            )
            return host, port, password

        # Fallback: user-stored settings
        host = str(self._repo.get("host") or "localhost")
        port = int(self._repo.get("port") or 4455)
        password = str(self._repo.get("password") or "")
        return host, port, password
