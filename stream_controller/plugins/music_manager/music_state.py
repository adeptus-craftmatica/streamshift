...
class MusicState:
    ...
    def play(self) -> None:
        if self._state.status == PlaybackStatus.PAUSED:
            ...
            # Nothing loaded — fall back to the library
            tracks = self._library.tracks
            if tracks:
                if self._state.shuffle:
                    self.play_random()
                else:
                    self.play_queue([t.path for t in tracks])
    ...
    def play_random(self) -> None:
        """Pick a random track from the library and play it immediately."""
        tracks = self._library.tracks
        if not tracks:
            return
    ...
    def remove_from_queue(self, idx: int) -> None:
        """Remove the track at the given queue index. Adjusts queue_index so current track is unaffected."""
        if not (0 <= idx < len(self._state.queue)):
            return
        self._state.queue.pop(idx)
        if idx < self._state.queue_index:
            self._state.queue_index -= 1
        elif idx == self._state.queue_index:
            # Removed the currently playing track — clamp to valid range
            self._state.queue_index = min(self._state.queue_index, len(self._state.queue) - 1)
        self._notify()

    def queue_next(self, paths: list[Path]) -> None:
        """Insert tracks immediately after the current position in the queue."""
        if not self._state.queue:
            self.play_queue(paths)
            return
        insert_at = self._state.queue_index + 1
        for i, p in enumerate(paths):
            self._state.queue.insert(insert_at + i, p)
        self._notify()

    def queue_last(self, paths: list[Path]) -> None:
        """Append tracks to the end of the current queue."""
        if not self._state.queue:
            self.play_queue(paths)
            return
        self._state.queue.extend(paths)
        self._notify()
