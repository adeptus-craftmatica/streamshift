...
<text x="40" y="44" text-anchor="middle" font-size="4.5" fill="rgba(63,148,191,0.65)">Artist</text>
    ...
    def _on_library_context_menu(self, pos) -> None:
        ...
        path = item.data(0, Qt.UserRole)
        if path is None:
            return  # clicked on artist/album node, not a track

        menu = QMenu(self._library_tree)
        edit_action = menu.addAction("Edit Metadata…")
        action = menu.exec(self._library_tree.viewport().mapToGlobal(pos))
        if action == edit_action:
            self._edit_track_metadata(path)
