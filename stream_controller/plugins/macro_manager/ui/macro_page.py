...
class MacroPage(QWidget):
    def __init__(
        ...
        # Four columns — single flat horizontal splitter, no nesting.
        # Col 1: macro list  Col 2: metadata  Col 3: steps  Col 4: step library
    ...
    def _build_step_library_card(self) -> QFrame:
        ...
        # Step type list — fixed reasonable height, not expandable
        ...
        # Param editor — sizes to content, scrolls if needed
    ...
    def _on_existing_step_selected(self, current: QListWidgetItem | None, _prev) -> None:
        ...
        # Find and select the matching type in the library list (suppressing its signal)
        ...
        # Rebuild the param editor for this type then fill in existing values
    ...
    def _add_step(self) -> None:
        ...
        if self._editing_step_row >= 0:
            # Update existing step in place, preserving its step_id
            ...
            # Stay in edit mode — user can keep tweaking
        else:
            step = MacroStep(
...
class _LibraryMultiPickerWidget(QWidget):
    """
    Full-width widget for the 'Choose Music' step.
    Shows selected tracks in a reorderable list; 'Add Tracks' opens the
    library picker dialog so the user never has to touch the filesystem.
    """
...
class _LibraryPickerDialog(QDialog):
    """Browse tracks already in the Music Manager library and pick one or more."""
    ...
    def __init__(self, app_context, parent: QWidget | None = None) -> None:
        ...
        self._app_context = app_context
        self._all_tracks: list = []  # list[Track]
...
class _RaidTargetWidget(QWidget):
    """Editable combo + saved favourites list for the Raid Channel step."""
