...
# Nav list row roles
_ROLE_PAGE_INDEX = Qt.UserRole        # int → stacked widget page index
_ROLE_PLUGIN_ID  = Qt.UserRole + 1   # str → plugin_id (plugin pages only)
_ROLE_NAV_TYPE   = Qt.UserRole + 2   # "page" | "header"

# Fixed stack page indices
_PAGE_HOTKEYS   = 0
_PAGE_RAID      = 1
_PAGE_BACKUP    = 2
_FIRST_PLUGIN_PAGE = 3
...
class PluginSettingsPage(QWidget):
    ...
    _import_done = Signal(int, int, object)
    ...
    def __init__(self) -> None:
        ...
        title.setContentsMargins(18, 0, 18, 12)
        sidebar_layout.addWidget(title)

        self._nav = QListWidget()
        self._nav.setObjectName("SidebarNavList")
        ...
        # ── Right panel: stacked content + footer ────────────────────────────
        right = QWidget()
        ...
        self._footer_reset_btn = QPushButton("Reset to Defaults")
        self._footer_reset_btn.setObjectName("SecondaryButton")
        self._footer_reset_btn.clicked.connect(self._on_footer_reset)

        self._footer_save_btn = QPushButton("Save")
        self._footer_save_btn.setObjectName("PrimaryButton")
        self._footer_save_btn.setMinimumWidth(110)
        self._footer_save_btn.clicked.connect(self._on_footer_save)

        footer_layout.addWidget(self._footer_reset_btn)
        footer_layout.addWidget(self._footer_save_btn)
        right_layout.addWidget(footer)

        root.addWidget(right, 1)

        # ── Build fixed pages ─────────────────────────────────────────────────
        self._stack.addWidget(self._wrap_in_scroll(self._build_hotkeys_card()))   # 0
        self._stack.addWidget(self._wrap_in_scroll(self._build_raid_list_card())) # 1
        self._stack.addWidget(self._wrap_in_scroll(self._build_backup_card()))    # 2

        # ── Populate nav ──────────────────────────────────────────────────────
        self._add_nav_header("GENERAL")
        self._add_nav_page("Hotkeys",   _PAGE_HOTKEYS)
        self._add_nav_page("Raid List", _PAGE_RAID)
        self._add_nav_page("Backup",    _PAGE_BACKUP)
        self._plugins_header_row: int | None = None  # set in set_settings
    ...
    def _add_nav_header(self, text: str) -> int:
        item = QListWidgetItem(text)
        item.setData(_ROLE_NAV_TYPE, "header")
        item.setFlags(Qt.NoItemFlags)
        item.setForeground(Qt.gray)
        self._nav.addItem(item)
        return self._nav.count() - 1

    def _add_nav_page(self, text: str, page_index: int, plugin_id: str = "") -> int:
        item = QListWidgetItem(text)
        item.setData(_ROLE_NAV_TYPE, "page")
        item.setData(_ROLE_PAGE_INDEX, page_index)
        item.setData(_ROLE_PLUGIN_ID, plugin_id)
        self._nav.addItem(item)
        return self._nav.count() - 1
    ...
    def _current_page_index(self) -> int | None:
        ...
        if item is None:
            return None
        if item.data(_ROLE_NAV_TYPE) != "page":
            return None
        return item.data(_ROLE_PAGE_INDEX)

    def _current_plugin_id(self) -> str | None:
        item = self._nav.currentItem()
        if item is None:
            return None
        pid = item.data(_ROLE_PLUGIN_ID)
        return pid or None

    def _on_nav_changed(self, row: int) -> None:
        item = self._nav.item(row)
        if item is None or item.data(_ROLE_NAV_TYPE) != "page":
            # Skip header rows — move to next selectable item
            for offset in (1, -1):
                candidate = self._nav.item(row + offset)
                if candidate and candidate.data(_ROLE_NAV_TYPE) == "page":
                    self._nav.setCurrentRow(row + offset)
                    return
            return
        page = item.data(_ROLE_PAGE_INDEX)
        if page is not None:
    ...
    def _sync_footer(self) -> None:
        ...
        if page == _PAGE_HOTKEYS:
            self._footer_save_btn.setText("Save Hotkeys")
            self._footer_reset_btn.setText("Reset to Suggested")
            self._footer_save_btn.setVisible(True)
            self._footer_reset_btn.setVisible(True)
        elif page is not None and page >= _FIRST_PLUGIN_PAGE:
            self._footer_save_btn.setText("Save Settings")
            self._footer_reset_btn.setText("Reset to Defaults")
            self._footer_save_btn.setVisible(True)
            self._footer_reset_btn.setVisible(True)
        else:
            # Raid list / Backup pages — no save/reset needed
            self._footer_save_btn.setVisible(False)
            self._footer_reset_btn.setVisible(False)

    def _on_footer_save(self) -> None:
        page = self._current_page_index()
        if page == _PAGE_HOTKEYS:
            self.hotkeys_save_requested.emit(self.current_hotkey_values())
        elif page is not None and page >= _FIRST_PLUGIN_PAGE:
            pid = self._current_plugin_id()
            if pid:
                self.save_requested.emit(pid, self.current_plugin_values(pid))

    def _on_footer_reset(self) -> None:
        page = self._current_page_index()
        if page == _PAGE_HOTKEYS:
            self.hotkeys_reset_requested.emit(list(self._hotkey_actions))
        elif page is not None and page >= _FIRST_PLUGIN_PAGE:
            pid = self._current_plugin_id()
            if pid:
                self.reset_requested.emit(pid)
    ...
    def set_settings(
        ...
        # Remove existing plugin pages from the stack (keep fixed pages 0-2)
        while self._stack.count() > _FIRST_PLUGIN_PAGE:
            page = self._stack.widget(_FIRST_PLUGIN_PAGE)
            self._stack.removeWidget(page)
            page.deleteLater()

        # Remove plugin nav items (everything after the plugins header row)
        if self._plugins_header_row is not None:
            while self._nav.count() > self._plugins_header_row + 1:
                self._nav.takeItem(self._nav.count() - 1)
        else:
            self._plugins_header_row = self._add_nav_header("PLUGINS")
        ...
        for plugin_id, definitions in sorted(
            ...
            page_index = self._stack.count()
            card = self._build_plugin_card(
                plugin_id=plugin_id,
                definitions=definitions,
                settings_manager=settings_manager,
            )
            self._stack.addWidget(self._wrap_in_scroll(card))

            display_name = definitions[0].plugin_name or plugin_id
            self._add_nav_page(display_name, page_index, plugin_id=plugin_id)

        # Show plugins header only when there are plugins
        if self._plugins_header_row is not None:
            header_item = self._nav.item(self._plugins_header_row)
            if header_item:
                has_plugins = self._nav.count() > self._plugins_header_row + 1
                header_item.setHidden(not has_plugins)
    ...
    def _build_hotkeys_card(self) -> QFrame:
        ...
        # Inline save/reset kept for the card (footer duplicates these, but
        # the card buttons provide a second affordance closer to the content)
    ...
    def _update_hotkey_feedback(self) -> None:
        ...
        if not self._hotkey_actions:
            ...
            self._hotkey_save_button.setEnabled(False)
            self._hotkey_reset_button.setEnabled(False)
            self._footer_save_btn.setEnabled(False)
            return
        ...
        if conflict_count:
            ...
            self._hotkey_save_button.setEnabled(True)
            self._footer_save_btn.setEnabled(True)
    ...
    def _do_export(self) -> None:
        ...
        def _run() -> None:
            try:
                ...
                files, secrets = export_backup(Path(dest), passphrase)
    ...
    def _do_import(self) -> None:
        ...
        def _run() -> None:
            try:
                ...
                files, secrets = import_backup(Path(src), passphrase)
