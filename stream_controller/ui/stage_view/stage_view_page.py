...
class StageViewPage(QWidget):
    ...
    def __init__(self, registry: "StageRegistry") -> None:
        ...
        self._logical_geos: dict[str, tuple[int, int, int, int]] = {}  # panel_id → (x,y,w,h) at zoom 1.0
        self._zoom = 1.0
        ...
        # Canvas widget: all panels live here at logical (1×) coordinates.
        # A QGraphicsView wraps it as a single proxy so view.scale() visually
        # scales the entire canvas including panel content — no per-panel trickery.
        ...
        self._canvas_proxy = self._scene.addWidget(self._canvas)
    ...
    def _build_toolbar(self) -> QWidget:
        ...
        fit_btn.setObjectName("StageToolbarBtn")
        fit_btn.setToolTip("Scroll to show all panels")
        ...
        zoom_lbl.setObjectName("StageToolbarLabel")
        layout.addWidget(zoom_lbl)

        zoom_out = QPushButton("−")
        zoom_out.setObjectName("StageToolbarBtn")
        zoom_out.setFixedWidth(28)
        zoom_out.setToolTip("Zoom out (10%)")
        zoom_out.clicked.connect(lambda: self._zoom_slider.setValue(self._zoom_slider.value() - 10))
        layout.addWidget(zoom_out)

        self._zoom_slider = QSlider(Qt.Horizontal)
        self._zoom_slider.setRange(30, 150)
        self._zoom_slider.setValue(100)
        self._zoom_slider.setFixedWidth(130)
        self._zoom_slider.setToolTip("Canvas zoom — drag to resize")
        self._zoom_slider.valueChanged.connect(self._on_zoom_changed)
        layout.addWidget(self._zoom_slider)

        zoom_in = QPushButton("+")
        zoom_in.setObjectName("StageToolbarBtn")
        zoom_in.setFixedWidth(28)
        zoom_in.setToolTip("Zoom in (10%)")
        zoom_in.clicked.connect(lambda: self._zoom_slider.setValue(self._zoom_slider.value() + 10))
        layout.addWidget(zoom_in)

        self._zoom_label = QLabel("100%")
        self._zoom_label.setObjectName("StageToolbarLabel")
    ...
    def _on_edit_toggled(self, checked: bool) -> None:
        ...
        # Keep the main toolbar button in sync even when toggled from the
        # fullscreen overlay button (they share this handler but are separate widgets).
    ...
    def _on_zoom_changed(self, value: int) -> None:
        ...
        for panel in self._panels.values():
            panel.set_zoom(self._zoom)
        if self._layout_restored:
            self._save_layout()

    def _fit_view(self) -> None:
        """Scroll to show all placed panels with a small margin."""
        if not self._panels:
            self._view.horizontalScrollBar().setValue(0)
            self._view.verticalScrollBar().setValue(0)
            return

        min_x = min(p.x() for p in self._panels.values())
        min_y = min(p.y() for p in self._panels.values())

        margin = 40
        self._view.horizontalScrollBar().setValue(max(0, int(min_x * self._zoom) - margin))
        self._view.verticalScrollBar().setValue(max(0, int(min_y * self._zoom) - margin))
    ...
    def _add_panel(self, panel_id: str, x: int | None = None, y: int | None = None,
        ...
        # x/y/w/h are always in logical (1× zoom) coordinates
        lx = x if x is not None else min(offset + 20, _CANVAS_W - dw - 20)
        ly = y if y is not None else min(offset + 20, _CANVAS_H - dh - 20)
        lw = w if w is not None else dw
        lh = h if h is not None else dh
        self._logical_geos[panel_id] = (lx, ly, lw, lh)
        ...
        panel.set_zoom(self._zoom)
        panel.setGeometry(lx, ly, lw, lh)
        panel.close_requested.connect(self._remove_panel)
        panel.geometry_changed.connect(lambda pid=panel_id: self._on_panel_geo_changed(pid))
    ...
    def _on_panel_geo_changed(self, panel_id: str) -> None:
        panel = self._panels.get(panel_id)
        if panel:
            g = panel.geometry()
            self._logical_geos[panel_id] = (g.x(), g.y(), g.width(), g.height())
        self._save_layout()

    def _remove_panel(self, panel_id: str) -> None:
        panel = self._panels.pop(panel_id, None)
        self._logical_geos.pop(panel_id, None)
        if panel:
            panel.destroy_grips()
            panel.hide()
    ...
    def _save_layout(self) -> None:
        ...
        for panel_id, panel in self._panels.items():
            d = panel.serialise()
            # Always save logical (1× zoom) coordinates so the layout is
            # zoom-independent and restores correctly at any zoom level.
            if panel_id in self._logical_geos:
                lx, ly, lw, lh = self._logical_geos[panel_id]
                d.update({"x": lx, "y": ly, "w": lw, "h": lh})
            data.append(d)
        self._layout_db.save(data, zoom=int(self._zoom * 100))

    def _restore_layout(self) -> None:
        saved_zoom = self._layout_db.zoom
        if saved_zoom != 100:
            self._zoom_slider.setValue(saved_zoom)  # triggers _on_zoom_changed
    ...
    def _launch_fullscreen(self, screen_index: int) -> None:
        ...
        win_layout.addWidget(self._view, 1)
    ...
    def _exit_fullscreen(self) -> None:
        ...
        self.layout().insertWidget(1, self._view, 1)
        self._view.show()
        self.layout().activate()
