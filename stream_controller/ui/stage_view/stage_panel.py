...
class _Grip(QWidget):
    ...
    def mouseMoveEvent(self, event) -> None:
        ...
        if d & _E:  right  = _snap(right  + delta.x())
        if d & _W:  left   = _snap(left   + delta.x())
        if d & _S:  bottom = _snap(bottom + delta.y())
        if d & _N:  top    = _snap(top    + delta.y())
...
class _TitleBar(QFrame):
    ...
    def mouseMoveEvent(self, event) -> None:
        if self._drag_start is None:
            return
        delta = event.globalPosition().toPoint() - self._drag_start
        new_x = _snap(self._orig_pos.x() + delta.x())
        new_y = _snap(max(0, self._orig_pos.y() + delta.y()))
